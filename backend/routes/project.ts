    ...
    .then(async (parsed) =>
      ormPr
        ...
        .then((obj) =>
          Promise.all(
            obj.map(async (val) => {
              const students = await ormCtr.contractsByProject(val.project_id);
        ...
        .then((obj) =>
          Promise.resolve({ sessionkey: parsed.data.sessionkey, data: obj })
        )
    );
}
    ...
    .then(async (rest) => {
      ...
      return ormPrRole
        ...
        .then((n) => {
          ...
          return Promise.resolve({ count: n, role: rest[0].project_role_id });
        });
    });
}
  ...
  return rq
    ...
    .then(async (parsed) => {
      ...
      return ormCtr
        ...
        .then((arr) =>
          arr.filter((v) => v.student.student_id == parsed.data.studentId)
        )
        .then((arr) => {
          if (arr.length == 0) {
            return Promise.reject({
        ...
        .then(async (ctr) => {
          return getFreeSpotsFor(parsed.data.role, parsed.data.id)
            .catch(() => createProjectRoleFor(parsed.data.id, parsed.data.role))
            .then((remaining) => {
  ...
  return rq
    ...
    .then(async (checked) => {
      return ormCtr
        ...
        .then((ctrs) =>
          ctrs.filter(
            (contr) => contr.project_role.project_id == checked.data.id
          )
        )
        .then(async (found) => {
          if (found.length == 0) {
            ...
            await ormEv
              ...
              .then((evl) => {
                ...
                return ormEv.updateEvaluationForStudent({
                  ...
                  motivation:
                    util.getOrDefault(evl[0].motivation, "") +
  ...
  return rq
    ...
    .then(async (checked) => {
      return ormOsoc
        ...
        .then((arr) =>
          Promise.resolve({ sessionkey: checked.data.sessionkey, data: arr })
        );
    });
}
