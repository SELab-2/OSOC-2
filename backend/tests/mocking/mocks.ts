...
class Callback {
    ...
    public set(cb: Call) {
        if (this.cb === undefined) this.cb = cb;
    }
}
...
export class MockedRouter
    extends CallableInstance<[express.Request, express.Response], Promise<void>>
    implements express.Router
{
    ...
    get = ((pth: string, rt: express.RequestHandler) => {
        this.verifier(pth);
        this.callbacks[pth].get.set(async (req, res, next) => {
    ...
    post = ((pth: string, rt: express.RequestHandler) => {
        this.verifier(pth);
        this.callbacks[pth].post.set(async (req, res, next) => {
            await rt(req, res, next);
        });
    ...
    put = ((pth: string, rt: express.RequestHandler) => {
        this.verifier(pth);
        this.callbacks[pth].put.set(async (req, res, next) => {
            await rt(req, res, next);
        });
    ...
    delete = ((pth: string, rt: express.RequestHandler) => {
        this.verifier(pth);
        this.callbacks[pth].delete.set(async (req, res, next) => {
            await rt(req, res, next);
        });
    ...
    patch = ((pth: string, rt: express.RequestHandler) => {
        this.verifier(pth);
        this.callbacks[pth].patch.set(async (req, res, next) => {
            await rt(req, res, next);
        });
...
export function getMockRouter(): MockedRouter {
    return new MockedRouter();
}
...
export async function expectRouter(
    router: MockedRouter,
    path: string,
    method: Method,
    req: express.Request,
...
export async function expectRouterThrow<T>(
    router: MockedRouter,
    path: string,
    method: Method,
    req: express.Request,
    ...
    req.method = method;
    return expect(
        expectRouter(router, path, method, req, res)
