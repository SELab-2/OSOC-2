...
import {
   createLanguage, deleteLanguage, deleteLanguageByName,
   getAllLanguages,
   getLanguage,
   getLanguageByName,
   updateLanguage
...
test("should update the given language", async () => {
   ...
   await expect(updateLanguage({languageId: 0, name: "updated"})).resolves.toEqual(returnValue);
});
...
test("should delete the given language name and its record and return the record", async () => {
   prismaMock.language.delete.mockResolvedValue(returnValue);
   await expect(deleteLanguageByName("languageName")).resolves.toEqual(returnValue);
})