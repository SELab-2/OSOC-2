...
it("should create 1 new project role with role developer", async () => {
    ...
    if (role) {
        ...
        expect(created_project_role).toHaveProperty(
            "project_id",
            projectRole1.projectId
        );
        expect(created_project_role).toHaveProperty(
            "role_id",
            projectRole1.roleId
        ...
        expect(created_project_role).toHaveProperty(
            "positions",
            projectRole1.positions
        );
    }
});

it("should return the project role, by searching for its project", async () => {
    const searched_project_role = await getProjectRolesByProject(
        projectRole1.projectId
    );
    expect(searched_project_role[2]).toHaveProperty(
        "project_id",
        projectRole1.projectId
    );
    expect(searched_project_role[2]).toHaveProperty(
...
it("should return the project role, by searching for its project and projectrole", async () => {
    const searched_project_role = await getNumberOfRolesByProjectAndRole(
        projectRole1.projectId,
        projectRole1.projectRoleId
    );
    expect(searched_project_role[0]).toHaveProperty(
        "project_id",
        projectRole1.projectId
    );
    expect(searched_project_role[0]).toHaveProperty(
...
it("should return the project role and role, by searching for its project role id", async () => {
    const searched_project_role = await getProjectRoleNamesByProject(
        projectRole1.projectId
    );
    expect(searched_project_role[2]).toHaveProperty(
        "project_id",
...
it("should return the the number of free positions, by searching for its project role id", async () => {
    const number_of_positions = await getNumberOfFreePositions(
        projectRole1.projectRoleId
    );
...
it("should update projectrole based upon project role id", async () => {
    ...
    expect(updated_project).toHaveProperty(
        "project_id",
        projectRole2.projectId
    );
...
it("should delete the project role based upon project role id", async () => {
    ...
    expect(deleted_project).toHaveProperty(
        "project_id",
        projectRole2.projectId
    );
