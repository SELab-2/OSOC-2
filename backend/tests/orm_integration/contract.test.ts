...
import {createContract, updateContract, removeContract,
    removeContractsFromStudent} from "../../orm_functions/contract";
...
it('should create 1 new contract linked to a student', async () => {
    ...
    if (student && project_role && login_user){
        ...
        expect(created_contract).toHaveProperty("contract_id", contract1.contractId);
        expect(created_contract).toHaveProperty("information", contract1.information);
        expect(created_contract).toHaveProperty("created_by_login_user_id", contract1.loginUserId);
        expect(created_contract).toHaveProperty("contract_status", contract1.contractStatus);
    }
});
...
it('should create 1 new contract linked to a student', async () => {
    ...
    if (student && project_role && login_user){
        ...
        const created_contract = await createContract(contract);
        expect(created_contract).toHaveProperty("information", contract.information);
        expect(created_contract).toHaveProperty("created_by_login_user_id", contract.loginUserId);
        expect(created_contract).toHaveProperty("contract_status", contract.contractStatus);
    }
});
...
it('should update contract based upon contract id', async () => {
    ...
    expect(updated_contract).toHaveProperty("created_by_login_user_id", contract2.loginUserId);
    expect(updated_contract).toHaveProperty("contract_status", contract2.contractStatus);
});
...
it('should delete the contract based upon contract id', async () => {
    ...
    expect(deleted_contract).toHaveProperty("created_by_login_user_id", contract2.loginUserId);
    expect(deleted_contract).toHaveProperty("contract_status", contract2.contractStatus);
});

it('should delete the contracts based upon student', async () => {
    const students = await prisma.student.findMany()
    const deleted_contracts = await removeContractsFromStudent(students[0].student_id);
