...
it("should update the email status of the job application", async () => {
    ...
    expect(changed).toHaveProperty("student_id", applic.student_id);
    expect(changed).toHaveProperty("student_volunteer_info", applic.student_volunteer_info);
    expect(changed).toHaveProperty("responsibilities", applic.responsibilities);
    expect(changed).toHaveProperty("fun_fact", applic.fun_fact);
    expect(changed).toHaveProperty("student_coach", applic.student_coach);
    expect(changed).toHaveProperty("osoc_id", applic.osoc_id);
    expect(changed).toHaveProperty("edus", applic.edus);
    expect(changed).toHaveProperty("edu_level", applic.edu_level);
    expect(changed).toHaveProperty("edu_duration", applic.edu_duration);
    expect(changed).toHaveProperty("edu_institute", applic.edu_institute);
    expect(changed).toHaveProperty("edu_year", applic.edu_year);
    expect(changed).toHaveProperty("created_at", applic.created_at);
...
it("should delete the job application", async () => {
    ...
    expect(deleted).toHaveProperty("student_id", app.student_id);
    expect(deleted).toHaveProperty("student_volunteer_info", app.student_volunteer_info);
    expect(deleted).toHaveProperty("responsibilities", app.responsibilities);
    expect(deleted).toHaveProperty("fun_fact", app.fun_fact);
    expect(deleted).toHaveProperty("student_coach", app.student_coach);
    expect(deleted).toHaveProperty("osoc_id", app.osoc_id);
    expect(deleted).toHaveProperty("edus", app.edus);
    expect(deleted).toHaveProperty("edu_level", app.edu_level);
    expect(deleted).toHaveProperty("edu_duration", app.edu_duration);
    expect(deleted).toHaveProperty("edu_institute", app.edu_institute);
    expect(deleted).toHaveProperty("edu_year", app.edu_year);
    expect(deleted).toHaveProperty("created_at", app.created_at);
...
it("should create a new job_application", async () => {
    ...
    expect(created).toHaveProperty("student_id", app.student_id);
    expect(created).toHaveProperty("student_volunteer_info", app.student_volunteer_info);
    expect(created).toHaveProperty("responsibilities", app.responsibilities);
    expect(created).toHaveProperty("fun_fact", app.fun_fact);
    expect(created).toHaveProperty("student_coach", app.student_coach);
    expect(created).toHaveProperty("osoc_id", app.osoc_id);
    expect(created).toHaveProperty("edus", app.edus);
    expect(created).toHaveProperty("edu_level", app.edu_level);
    expect(created).toHaveProperty("edu_duration", app.edu_duration);
    expect(created).toHaveProperty("edu_institute", app.edu_institute);
    expect(created).toHaveProperty("edu_year", app.edu_year);
    expect(created).toHaveProperty("created_at", app.created_at);
...
it("should return the most recent job application of a student", async () => {
    ...
    const attachments = await prisma.attachment.findMany({
        where: {
            job_application_id: found?.job_application_id
        }
    });

    const applied_roles = await prisma.applied_role.findMany({
        where: {
            job_application_id: found?.job_application_id
        }
    });

    const job_application_skill = await prisma.job_application_skill.findMany({
        where: {
            job_application_id: found?.job_application_id
        }
    });
