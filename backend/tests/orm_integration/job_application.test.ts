...
function job_application_check(
    ...
    expect(found).toHaveProperty(
        ...
        expected.student_volunteer_info
    );
...
it("should return all student evaluations for the student with given id", async () => {
    ...
    const foundApplications = await getStudentEvaluationsTotal(
    ...
    foundApplications.forEach((found_eval) => {
        ...
        for (let i = 0; i < found_eval.evaluation.length; i++) {
            ...
            expect(found_eval.evaluation[i]).toHaveProperty(
                "decision",
                evals.decision
            );
            expect(found_eval.evaluation[i]).toHaveProperty(
...
it("should return all final student evaluations for the student with given id", async () => {
    ...
    const foundApplications = await getStudentEvaluationsFinal(
        students[0].student_id
    );
    foundApplications.forEach((found_eval) => {
        expect(found_eval).toHaveProperty("osoc", { year: osocs[0].year });
...
it("should return all suggestion evaluations for the student with given id", async () => {
    ...
    const foundApplications = await getStudentEvaluationsTemp(
        students[0].student_id
    );
    foundApplications.forEach((found_eval) => {
        expect(found_eval).toHaveProperty("osoc", { year: osocs[0].year });
...
it("should return the last roles a student applied for", async () => {
    ...
    const applied_roles = await getLatestApplicationRolesForStudent(
        students[0].student_id
    );
    if (applied_roles) {
        ...
        expect(applied_roles.applied_role.length).toEqual(
            roles_expected.length
        );
    }
});
...
it("should delete all the job applications of the given student", async () => {
    ...
    const deleted = await deleteJobApplicationsFromStudent(
        students[2].student_id
    );
 ...
 *
 */
function getDataAssociatedWithApplication(
    job_application_id: number | undefined
) {
    return Promise.all([
