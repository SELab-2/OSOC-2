...
it("should delete the applied roles for the job application", async () => {
    const [job_applications, roles] = await Promise.all([prisma.job_application.findMany(), prisma.role.findMany()])

    const deleted = await deleteAppliedRolesByJobApplication(job_applications[1].job_application_id);
