...
it("should create a new session key for the given login user", async () =>{
    ...
    const created = await addSessionKey(loginUsers[0].login_user_id, newKey, futureDate);
...
it("should overwrite the old key with the new key", async () => {
    ...
    const updated = await changeSessionKey(existing_keys[0].session_key, newkey, futureDate);
