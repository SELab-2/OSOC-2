...
it("should create 1 new attachment linked to a job application", async () => {
    ...
    if (job_application) {
        const created_attachment = await createAttachment(
            job_application.job_application_id,
            attachment1.data,
            attachment1.type as type_enum[]
        );
...
it("should return the attachment, by searching for its attachment id", async () => {
    const searched_attachment = await getAttachmentById(
...
it("should delete the attachment based upon job application id", async () => {
    const deleted_attachments = await deleteAllAttachmentsForApplication(
        attachment2.jobApplicationID
    );
