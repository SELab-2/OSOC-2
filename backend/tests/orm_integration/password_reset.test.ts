...
import {createOrUpdateReset, findResetByCode, deleteResetWithLoginUser,
    deleteResetWithResetId} from "../../orm_functions/password_reset";
...
it("should create a new password resety for the given login user", async () =>{
    ...
    if (loginUser){      
        const created_password_reset = await createOrUpdateReset(loginUser.login_user_id, newReset, date);

        expect(created_password_reset).toHaveProperty("login_user_id", loginUser.login_user_id);
...
it('should delete the password reset based upon login user id', async () => {
    ...
    if (loginUsers[2] && password_reset){       
        const deleted_password_reset = await deleteResetWithLoginUser(loginUsers[2].login_user_id);
        expect(deleted_password_reset).toHaveProperty("reset_id", password_reset.reset_id);
        expect(deleted_password_reset).toHaveProperty("valid_until", password_reset.valid_until);
    }
});
