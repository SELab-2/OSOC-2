...
test("Can list all coaches", async () => {
  ...
  await expect(coach.listCoaches(req))
...
test('Getting a single coach is deprecated.', async () => {
  await expect(coach.getCoach(getMockReq()))
...
test("Can modify a single coach (1).", async () => {
  ...
  expectCall(utilMock.checkSessionKey,
             {id : 7, pass : 'jeff', sessionkey : 'abcd'});
...
test("Can get coach requests", async () => {
  const req = getMockReq();
  const res_ = people.filter(v => v.is_coach && v.account_status == 'PENDING')
                   .map(v => ({
                          person_data : {
...
test('Getting a single coach request is deprecated.', async () => {
  await expect(coach.getCoachRequest(getMockReq()))
      .rejects.toStrictEqual({http : 410, reason : 'Deprecated endpoint.'});
