...
import {getNumberOfFreePositions, createProjectRole, getProjectRolesByProject,
    getNumberOfRolesByProjectAndRole, getProjectRoleNamesByProject,
    updateProjectRole, deleteProjectRole} 
...
test("should return the number of roles for the given project and project role", async () => {
    prismaMock.project_role.findMany.mockResolvedValue([returnValue]);
    await expect(getNumberOfRolesByProjectAndRole(0, 0)).resolves.toEqual([returnValue]);
});
