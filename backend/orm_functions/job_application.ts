...
export async function getLatestApplicationRolesForStudent(studentId: number) {
    return await prisma.job_application.findFirst({
        ...
        orderBy: {
            // use descending order in combination with findFirst to search further for the applied roles in the latest application
            created_at: "desc",
        },
        select: {
            applied_role: {
 ...
 */
export async function changeEmailStatusOfJobApplication(
