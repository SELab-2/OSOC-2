...
const Index: NextPage = () => {
    ...
    return (
        <div className={`${styles.students} ${display === Display.LIMITED ? styles.limited : ""}`}>
            <div className={`${styles.studentCards} ${display === Display.LIMITED ? styles.limited : ""}`}>
                {students.map((student, index) => {
                    return (<div key={student.student.student_id} className={styles.card}
                        ...
                        <StudentCard student={student as Student} display={display}/>
                        {student.evaluations[0].evaluation.length > 0 ?
                            <EvaluationBar evaluations={student.evaluations[0].evaluation}/> : null}
                    </div>)
                })}
            </div>
